# 📦 AZUSTOCK Co., Phy. 作品集與技術部落格 - 交接與維護指南 (U1.3)
...
## 8. 版本更新紀錄 (U1.3: Lightbox 2.0 與全域操作優化)

本次更新重點強化了「全螢幕大圖預覽 (Lightbox)」的互動體驗，並重構了全站的鍵盤監聽與響應式排版邏輯。

### 8.1 進階大圖預覽 (Lightbox 2.0) 引擎
*   **滑鼠追蹤縮放 (Cursor-Tracking Zoom)**：在 `main.js` 實作了基於滾輪 (`wheel`) 的縮放演算。利用 `(e.clientX - centerX) * ratio` 的平移補償公式，抵銷單純 `scale` 造成的座標偏移，達成「指哪裡、放哪裡」的專業軟體級縮放體驗。
*   **滑順拖曳平移 (Panning)**：加入滑鼠與觸控拖曳支援，並透過在 `mousedown` 時加上 `e.preventDefault()`，有效阻斷瀏覽器預設的「圖片拖曳存檔」殘影行為。
*   **瞬時狀態重置**：每次開啟 Lightbox 時，會強制清除舊的 `transform` (X, Y, Zoom) 狀態，並短暫關閉 `transition`，防止出現圖片從上次座標「飛回來」的殘影動畫。

### 8.2 響應式工具列與 UI 佈局優化
*   **手機版收合工具箱 (Collapsible Toolbox)**：在 `style.css` 中利用 Flexbox 與絕對定位，將工具列分為左右群組。手機版時，透過 `right: 85px; transform: translateX(50%)` 的精密幾何計算，讓橫向工具箱能完美疊合於「更多選項 (`...`)」按鈕的正中心展開，並利用 `@media` 隱藏觸控裝置的 Tooltip 以防止殘留擋圖。
*   **Flexbox 溢出防護**：修正了 `.lightbox-img-wrapper` 的高度計算陷阱，加上 `min-height: 0; min-width: 0; flex: 1;`，確保極端比例的圖片不會撐破 Flexbox 的預設高度限制而引發跑版。
*   **圖文視覺分離**：透過加大底部 `padding` (90px)，確保無論圖片如何縮放，都不會遮擋底部的半透明文字膠囊 (`lightbox-caption`)。
*   **SVG 細節回饋**：為「縮小按鈕 (`zoom-out`)」單獨加上反向的 hover 縮放動畫 (`scale(0.85)`)，營造按壓的物理回饋感。

### 8.3 統合型全域鍵盤引擎 (Unified Keyboard Engine)
*   **統一事件池**：移除了原本散落在各處的 `keydown` 監聽器，改為在 `main.js` 底部統一處理，利用狀態變數（如 `isLightboxOpen`, `isArticleOpen`）精準分發指令，徹底避免事件衝突。
*   **三段式退回邏輯 (ESC)**：針對 Lightbox 實作高階防呆機制。按下 `ESC` 時：
    1. 若手機版工具箱開啟 ➔ 先收起工具箱。
    2. 若圖片目前為放大狀態 ➔ 恢復 1:1 原始比例。
    3. 上述皆非 ➔ 才真正關閉 Lightbox。
*   **文章無縫切換**：在文章閱讀模式 (Modal 開啟) 下，監聽鍵盤 `←` 與 `→` 鍵，自動尋找並觸發底部的「上一篇/下一篇」導覽膠囊，實現純鍵盤的沉浸式連貫閱讀體驗。
*   **背景防滾動鎖定**：在 Lightbox 模式下主動攔截 `↑` 與 `↓` 鍵的預設行為，防止在看圖時，底層文章意外滾動導致迷失閱讀進度。