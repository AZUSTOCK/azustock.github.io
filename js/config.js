...
export const CONFIG = {
    ...
    // ✨ 動態判定基底路徑 (Base Path)
    get BASE_URL() {
        // 如果是本機 Live Server (127.0.0.1 或 localhost)，根目錄就是 "/"
        if (window.location.hostname === '127.0.0.1' || window.location.hostname === 'localhost') {
            return '';
        }
        // 如果是 GitHub Pages，且你的專案名稱是 "azustock.github.io" (User Page)，根目錄也是 ""
        // 如果你的專案有子目錄名稱 (例如 /repo-name/)，請在這裡填入 `/${window.location.pathname.split('/')[1]}`
        return ''; 
    },
